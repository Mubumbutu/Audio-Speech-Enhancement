...
title FlowHigh Super-Resolution - Installer
color 0A

echo.
echo ============================================================
echo  FlowHigh Super-Resolution - Dependency Installer
echo  Any SR input ^> 48 kHz output via single-step flow matching
echo  CPU and GPU supported, ~500 MB model
echo ============================================================
echo.

set ROOT_DIR=%~dp0..
set VENV_DIR=%ROOT_DIR%\venvs\venv_flowhigh
set REQ_FILE=%~dp0flowhigh_requirements.txt
...
:: ----------------------------------------------------------------
:: [2/6] Check required files
:: ----------------------------------------------------------------
echo [2/6] Checking required files...
if not exist "%REQ_FILE%" (
    echo [ERROR] flowhigh_requirements.txt not found at: %REQ_FILE%
    pause
    exit /b 1
)
echo [OK] flowhigh_requirements.txt found.
echo.
...
if exist "%VENV_DIR%" (
    echo [INFO] Removing existing venv_flowhigh...
    rmdir /s /q "%VENV_DIR%"
)
...
:: ----------------------------------------------------------------
:: [4/6] Detect NVIDIA GPU and select PyTorch variant
:: ----------------------------------------------------------------
echo [4/6] Detecting NVIDIA GPU...
set GPU_FOUND=0
set DRIVER_MAJOR=0
...
if not errorlevel 1 (
    set GPU_FOUND=1
    nvidia-smi --query-gpu=driver_version --format=csv,noheader > "%TEMP%\flowhigh_gpu_driver.txt" 2>nul
    for /f "tokens=1 delims=." %%a in (%TEMP%\flowhigh_gpu_driver.txt) do (
        set DRIVER_MAJOR=%%a
        goto :FH_DRIVER_DONE
    )
)
:FH_DRIVER_DONE
del "%TEMP%\flowhigh_gpu_driver.txt" >nul 2>&1

if !GPU_FOUND! EQU 0 (
    echo [INFO] No NVIDIA GPU detected. Installing CPU-only PyTorch.
    echo        FlowHigh runs on CPU - GPU gives faster inference.
    set TORCH_URL=https://download.pytorch.org/whl/cpu
    set TORCH_VARIANT=CPU only
    goto :FH_START_INSTALL
)
...
:: ----------------------------------------------------------------
:: [5/6] Upgrade pip, install PyTorch + requirements + FlowHigh
:: ----------------------------------------------------------------
echo [5/6] Upgrading pip and setuptools...
...
echo Installing dependencies from flowhigh_requirements.txt...
...
echo Installing FlowHigh from GitHub ^(dev branch^)...
python -m pip install "git+https://github.com/resemble-ai/flowhigh.git@dev" --no-deps
if errorlevel 1 (
...
echo [OK] FlowHigh installed.
echo.

echo Restoring PyTorch ^(!TORCH_VARIANT!^) after FlowHigh install...
...
python -c "import torchode; print('torchode: import OK')"
if errorlevel 1 (
    echo [ERROR] torchode failed to import.
    pause
    exit /b 1
)
...
python -c "from flowhigh import FlowHighSR; print('FlowHigh: import OK')"
if errorlevel 1 (
    echo [ERROR] FlowHigh failed to import.
    pause
    exit /b 1
)
echo.
...
echo ============================================================
echo  FlowHigh installation complete!
echo.
...
echo    2. Select "FlowHigh Super-Resolution" from the menu
echo    3. Click "Download Model" to fetch FlowHigh weights ^(~500 MB^)
echo    4. Drop an audio file and click "Enhance Audio"
echo.
echo  Note: FlowHigh supports any input sampling rate.
